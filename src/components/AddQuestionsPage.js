...
const AddQuestionsPage = () => {
	...
	const addQuestionSubmit = (e) => {
		e.preventDefault();
		if (
			[subjectName.trim(), temaName.trim(), question.trim(), correctAnswer.trim(), answer2.trim(), answer3.trim()].some(
				(item) => item.length <= 0
			)
		)
			return;
		...
		axios
			...
			.then(function (response) {
				responseForQuestionRef.current.innerHTML = "<pre>" + JSON.stringify(response.data, null, 2) + "</pre>";
			})
	...
	const addTemaSubmit = (e) => {
		e.preventDefault();
		if ([subjectName.trim(), temaName.trim()].some((item) => item.length <= 0) || !numero) return;
		const config = { headers: { Authorization: "Bearer " + JWTToken } };
		const myObj = {
			subject_name: subjectName.trim(),
			tema: temaName.trim(),
		...
		axios
			...
			.then(function (response) {
				responseForTemaRef.current.innerHTML = "<pre>" + JSON.stringify(response.data, null, 2) + "</pre>";
			})
			.catch(function (error) {
				console.log(error);
			});
	...
	const addSubjectSubmit = (e) => {
		...
		axios
			...
			.then(function (response) {
				responseForSubjectRef.current.innerHTML = "<pre>" + JSON.stringify(response.data, null, 2) + "</pre>";
	...
	const fetchData = async () => {
		try {
			...
			for (let subject of response.data) {
				jsonHtml += "<div><h4  style='display: inline'>" + subject.name + ": " + "</h4>";
				for (let tema of subject.temas) {
					jsonHtml += tema.name + ", ";
				}
				jsonHtml += "</div>";
	...
	return (
		<>
			<button style={{ zIndex: "100" }} onClick={() => setToLogin(!toLogin)}>
				Login
			</button>
			{toLogin && (
				<div className='background-cover' style={{ backgroudColor: "rgba(0,0,0,0.3)" }}>
					<form style={{ zIndex: "10" }} className='form-api' onSubmit={(e) => login_to_api(e)}>
			...
			<div className='btn-group' role='group' aria-label='Basic example'>
				<button type='button' className='btn btn-secondary' onClick={() => setTypeOfForm("add-question")}>
					Add question
				</button>
				<button type='button' className='btn btn-secondary' onClick={() => setTypeOfForm("add-tema")}>
					Add tema
				</button>
				<button type='button' className='btn btn-secondary' onClick={() => setTypeOfForm("add-subject")}>
					Add subject
				</button>
				<button type='button' className='btn btn-secondary' onClick={() => setTypeOfForm("get-all")}>
					Get all
				</button>
			</div>
			{typeOfForm == "add-question" ? (
				<div className='form-container'>
					<form className='form-api' onSubmit={(e) => addQuestionSubmit(e)}>
						<label className='form-text' htmlFor='subject_name'>
							Subject name
						</label>
						<input
							style={{ width: "30%", display: "inline", margin: "10px", marginRight: "50px" }}
						...
						<datalist id='subjects'>
							{data.map((subject, index) => {
								return <option value={subject.name} key={index} />;
							})}
						</datalist>

						<label className='form-text' htmlFor='tema'>
							Tema
						</label>
						<input
						...
						<datalist id='temas'>
							{temasOfSubjectSelected.map((tema, index) => {
								return <option value={tema.name} key={index} />;

								//<option value={subject.name} key={index} />;
							})}
						</datalist>
						<label className='form-text' htmlFor='Question' style={{ display: "block" }}>
							Question
						</label>
						<input
							className='form-control text-label'
						...
						<label className='form-text' htmlFor='Correct answer' style={{ color: "green" }}>
							Correct answer
						</label>
						<input
						...
						/>
						<label className='form-text' htmlFor='Wrong answers' style={{ color: "red" }}>
							Wrong Answers
						</label>
						<input
						...
						/>
						<div>
							<label className='' htmlFor='isDate' style={{ margin: "10px" }}>
								isDate
							</label>
							<input
								style={{ borderColor: "green" }}
						...
						<div style={{ textAlign: "left" }} ref={responseForQuestionRef}></div>
					</form>
				</div>
			) : typeOfForm == "add-tema" ? (
				<form className='form-api'>
					...
					<input
						...
						onChange={(e) => setTemaName(e.target.value)}
					/>
					<label className='form-text' htmlFor='numero'>
						numero
					</label>
					<input
						className='form-control text-label'
					...
					<div style={{ textAlign: "left" }} ref={responseForTemaRef}></div>
				</form>
				...
				<form className='form-api'>
					...
					<div style={{ textAlign: "left" }} ref={responseForSubjectRef}></div>
				</form>
			) : (
				<>
